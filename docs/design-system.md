...
### Programmatic Guide (`/guides/sos-lookup/[state]`)

**Job:** Rank for "[state] secretary of state entity search" and similar. Capture traffic from people doing manual lookups.

Structure:
1. Title: "[State] Secretary of State Entity Search: Guide for Lenders"
2. Breadcrumbs: Guides > SOS Lookup > [State]
3. Quick facts card: State portal URL, entity types tracked, online availability
4. Step-by-step instructions: Numbered, specific to that state's website
5. What you can find: Formation date, registered agent, good standing, filing history
6. Common issues: State-specific gotchas (delays, different terminology, etc.)
7. CTA card: "Skip the manual search. PulseClose checks [State] and 49 other states in seconds."

**Key:** Each page must have genuinely useful, state-specific content. Not just the same template with the state name swapped. The step-by-step instructions differ per state because every SOS website is different.

### Glossary Entry (`/glossary/[term]`)

**Job:** Rank for "what is [term]" queries. Build topical authority. Create internal linking hub.

Structure:
1. Term as H1
2. One-paragraph definition (clear, no jargon-to-explain-jargon)
3. "Why it matters for bridge lenders" section (2-3 paragraphs, practical)
4. Example if applicable
5. Related terms (linked)
6. CTA: Contextual, tied to the term (e.g., "Automate bankruptcy screening with PulseClose")
